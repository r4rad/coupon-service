...
function New-TicketPrompt {
    ...
    $P.Add("  6. Commit as `"$id`: <imperative summary>`", push the branch, and open a pull request titled")
    $P.Add("     `"$($id): $($Ticket.title)`".")
...
function New-TicketMarkdown {
    ...
    $L.Add('6. A pull request exists listing the acceptance criteria satisfied and anything deferred.')
    $L.Add('')
    $L.Add('---')
    $L.Add('')
