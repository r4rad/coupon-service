...
| P-14 | **Branching:** feature → `develop` (PR CI) → merge CD to non-prod RG; `develop` → `main` (PR CI) → merge CD to production RG. Same eight stages; branch selects RG + bicepparam. Live region for this subscription is **eastus2** (westeurope ineligible). **Ticket work for CS-29/CS-30 branches from `develop` and PRs target `develop`;** the operator merges `develop` → `main` separately | Client delivery wants a staging gate before production without a second pipeline file | CS-29; `rg-coupon-demo` (dev/demo) and `rg-coupon-prod`; param files pin `eastus2`; WIF SP needs Contributor + User Access Administrator on both RGs for Key Vault roleAssignments |
