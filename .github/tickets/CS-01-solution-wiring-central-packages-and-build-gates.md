...
| **Wave** | 0 — Foundations |
| **Size** | M |
| **Labels** | `wave-0, area:build, size:M` |
| **Blocked by** | — |
| **Blocks** | [CS-02](CS-02-domain-primitives-and-the-money-contract.md), [CS-03](CS-03-value-model-and-expression-ast.md), [CS-25](CS-25-bicep-modules-for-the-whole-environment.md) |
...
Bring the half-scaffolded repository to a clean, buildable, committed baseline. Eleven projects exist on disk but CouponService.slnx is empty, template files remain, and there are no commits yet.
...
- `tests/CouponService.UnitTests/Architecture/`
...
- Any production logic. This ticket only makes the skeleton build and enforce its own rules.
...
- Add all eleven projects under src/ and tests/ to CouponService.slnx.
- Delete template cruft: Class1.cs from every class library, and the weather forecast sample controller and model from both web API projects.
- Wire project references: Engine to Domain only; Application to Domain and Engine; Infrastructure to Application; Api to Application and Infrastructure; EngineTests to Engine; UnitTests to Application, Infrastructure and Engine; ApiTests to Api; IntegrationTests to Infrastructure. OrderApi and the Bdd project stay standalone and talk HTTP, which keeps the service boundary honest.
- Introduce Directory.Packages.props with ManagePackageVersionsCentrally, and move every version out of individual csproj files.
- Add an architecture test asserting that CouponService.Engine's transitive assembly references contain no ASP.NET Core, Azure or Cosmos assembly. Assert on the compiled assembly's referenced names so the rule cannot be bypassed by a csproj edit.
...
Paste this into a fresh Cursor chat in this repository. Nothing else is needed: everything is
either in the prompt or in a file the prompt names.

```text
Implement ticket CS-01 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-01-solution-wiring-central-packages-and-build-gates.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of NFR-7, AC-10.5. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-01-solution-wiring-central-packages from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-01: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-01: Solution wiring, central packages and build gates".
