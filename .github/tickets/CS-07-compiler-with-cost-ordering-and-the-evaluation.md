...
| **Size** | L |
| **Labels** | `wave-1, area:engine, size:L` |
| **Blocked by** | [CS-04](CS-04-policy-parser-with-depth-and-node-budgets.md), [CS-05](CS-05-fact-registry-fact-vocabulary-and-engine-manifest.md) |
| **Blocks** | [CS-08](CS-08-content-hashing-and-the-bounded-compiled-policy.md), [CS-09](CS-09-core-effect-handlers-and-the-price-calculator.md) |
...
Compile the AST to closures, ordering logical operands cheapest-first so short-circuiting avoids network round trips, and evaluate with memoised facts and a trace that captures near misses.

## Blocked by

- CS-04 — Policy parser with depth and node budgets
- CS-05 — Fact registry, fact vocabulary and engine manifest

## Scope — touch only these paths

- `src/CouponService.Engine/Compilation/`
- `src/CouponService.Engine/Evaluation/`
- `tests/CouponService.EngineTests/Compilation/`
...
- Caching compiled policies across requests, which is CS-08.
- Effects, which are CS-09.
...
- Compile to the Compiled delegate from design.md: closures over delegates, not System.Linq.Expressions, per decision P-6.
- Every compiled node returns its aggregate FactCost. Order the operands of all and any by ascending cost, which is sound because every operator is pure.
- EvalScope owns per-evaluation fact memoisation, the injected clock, the trace collector and the quantifier line scope.
- Capture near misses always, on every failed numeric comparison, recording actual, required and shortfall. Build the full node-by-node trace only when explicitly requested, per decision P-7.
- The headline test: register a counting fact provider for coupon.uses.total, evaluate a policy whose cheap subtotal predicate fails, and assert the counter was invoked exactly zero times. This proves the design claim rather than asserting it.
- Also test operand reordering, that a fact referenced three times resolves once, operator truth tables, and that two evaluations with a FixedClock produce identical output.
...
dotnet test tests/CouponService.EngineTests --filter FullyQualifiedName~Compilation
dotnet test CouponService.slnx
```

## Prompt

Confirm CS-04 and CS-05 are merged, then paste this into a fresh Cursor
chat in this repository. Nothing else is needed: everything is either in the prompt or in a
file the prompt names.

```text
Implement ticket CS-07 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-07-compiler-with-cost-ordering-and-the-evaluation.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-3.1, AC-3.2, AC-3.3, AC-1.3, AC-2.7, AC-3.7, AC-10.2. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-07-compiler-with-cost-ordering-and from the latest main.
  ...
  7. Push the branch and open a pull request against main titled "CS-07: Compiler with cost ordering, and the evaluation scope".
