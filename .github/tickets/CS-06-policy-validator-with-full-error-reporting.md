# CS-06: Policy validator with full error reporting
...
Reject an invalid policy at write time so the request path can trust every stored policy. This is what makes the engine safe to be data-driven.
...
- `src/CouponService.Engine/Validation/`
- `tests/CouponService.EngineTests/Validation/`
...
- The HTTP 400 shape, which is CS-17.
...
- Validate every FactExpr path against the registry, and reject unknown paths naming the path and its JSON location.
- Type-check comparisons, membership, arithmetic and aggregates against declared fact kinds, so a type error surfaces at write time and never at evaluation time.
- Validate quantifier scope: a line.* fact is legal only inside an every or some over cart.lines, and illegal at the top level.
- Validate engineSchema and refuse a version the engine does not implement.
- Report every offending node, not just the first. A campaign manager fixing one typo per round trip is a bad tool.
- Test each rejection class, and specifically assert that a document with three separate errors reports all three.
...
dotnet test tests/CouponService.EngineTests --filter FullyQualifiedName~Validation
...
```text
Implement ticket CS-06 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-06-policy-validator-with-full-error-reporting.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-2.3, AC-2.4, AC-2.10. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-06-policy-validator-with-full-error from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-06: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-06: Policy validator with full error reporting".
