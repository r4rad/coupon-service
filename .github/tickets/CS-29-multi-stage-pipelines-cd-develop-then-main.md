...
- Environments: develop CD uses environmentName=dev (or demo) and RG rg-coupon-demo (non-prod). main CD uses environmentName=prod and RG rg-coupon-prod. Add infra/bicep/main.dev.bicepparam and main.prod.bicepparam (no secrets). Map branch->RG+param file in YAML variables so an agent needs no manual override for the happy path. Both environments use hostingMode=containerApps; the subscription allows one CAE per region and zero App Service VMs, so main.prod.bicepparam sets containerAppsLocation=eastus - only the prod CAE and its apps run there, every other prod resource stays eastus2.
