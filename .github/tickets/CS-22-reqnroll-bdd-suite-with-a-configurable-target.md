...
Satisfy the brief's BDD requirement with scenarios that read as business behaviour, runnable locally now and against APIM later without editing a feature file.
...
- Running against deployed Azure, which happens once a subscription exists.
...
- Take base URL and token strategy from configuration, so the same features run against localhost today and APIM later. This is the whole reason the suite is deliverable without Azure.
- Implement the eleven scenarios in section 19 of docs/solution-architecture.md, including near-miss hints, a rule created at runtime with no redeployment, capped best-of, an automatic policy, an unknown fact rejected on write, concurrency, tampering and the 403 case.
- Seed every scenario's own policies through the admin API under a per-run prefix such as RUN7F3A_, and delete them in teardown. Never depend on pre-existing shared codes, which is precisely why the reference proposal's suite contradicts itself on a second run.
- Fail with a clear message if the target is unreachable, rather than reporting a misleading assertion failure.
...
Read these first, in order. They are the contract and they override anything you assume:
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-10.1, AC-10.4. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-22-reqnroll-bdd-suite-with-a-configurable from the latest main.
  ...
  3. You may add these packages, versions centralised in Directory.Packages.props: Reqnroll.xUnit
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-22: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-22: Reqnroll BDD suite with a configurable target".
