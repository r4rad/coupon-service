# CS-24: Cosmos emulator integration tests

| | |
|---|---|
| **Wave** | 6 — Cosmos adapter |
| **Size** | M |
| **Labels** | `wave-6, area:test, area:data, size:M` |
| **Blocked by** | [CS-23](CS-23-cosmos-db-adapter-with-transactional-reservation.md) |
| **Blocks** | — |
...
Exercise the real data access path against a real Cosmos engine, covering the behaviours only the database can prove.
...
- `tests/CouponService.IntegrationTests/`
- `docker-compose.yml`
...
- Changing production code, unless the emulator reveals a genuine bug, which should be fixed and called out.
...
- Add docker-compose.yml running the Cosmos emulator, with documented startup time and the certificate step.
- Skip the entire project with an explicit, readable message when the emulator is unreachable, so a developer without Docker still gets a green suite. Never let it fail silently or hang.
- Test the transactional batch actually rolls back, that a duplicate orderId violates the unique key, that a genuine ETag conflict returns 412, and that TTL is set while Reserved and cleared on confirm.
- Test true parallel reservations at a cap of one, from multiple concurrent tasks, and assert exactly one wins.
...
Read these first, in order. They are the contract and they override anything you assume:
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-10.6, AC-4.5. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-24-cosmos-emulator-integration-tests from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-24: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-24: Cosmos emulator integration tests".
