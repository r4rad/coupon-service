...
| **Size** | L |
| **Labels** | `wave-2, area:engine, area:domain, size:L` |
| **Blocked by** | [CS-02](CS-02-domain-primitives-and-the-money-contract.md), [CS-07](CS-07-compiler-with-cost-ordering-and-the-evaluation.md) |
| **Blocks** | [CS-10](CS-10-advanced-effect-handlers-cheapestfree-nthitem.md), [CS-12](CS-12-application-services-and-in-memory-repositories.md) |
...
Turn an effect expression into a discount plan allocated per line, including the composition operators that a discount enum could never express.

## Blocked by

- CS-02 — Domain primitives and the money contract
- CS-07 — Compiler with cost ordering, and the evaluation scope

## Scope — touch only these paths

- `src/CouponService.Engine/Effects/`
- `src/CouponService.Application/Pricing/`
...
- cheapestFree, nthItem and tiered, which are CS-10.
...
- Add IEffectHandler and EffectScope per design.md, plus selector evaluation so a where clause selects lines consistently with the condition grammar.
- Implement percentage, fixedAmount, sum, bestOf and cap.
- bestOf must compute every branch and select the largest total, never guess by inspecting shape.
- cap must clamp the total and rescale per-line allocations proportionally, re-rounding so allocations always sum exactly to the capped total. Test the rounding residue case where naive proportional scaling is one cent off.
- Implement PriceCalculator taking a decision, never a coupon code, so an unvalidated discount cannot be applied.
- Test the capped best-of example from the architecture document: 15 percent over a 29.00 vegetarian base against 5.00 flat, capped at 10.00.
...
Read these first, in order. They are the contract and they override anything you assume:
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-1.1, AC-1.4, AC-2.8, AC-2.9. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-09-core-effect-handlers-and-the-price from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-09: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-09: Core effect handlers and the price calculator".
