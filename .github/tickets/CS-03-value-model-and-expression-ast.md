...
| **Blocks** | [CS-04](CS-04-policy-parser-with-depth-and-node-budgets.md), [CS-05](CS-05-fact-registry-fact-vocabulary-and-engine-manifest.md) |
...
Define the typed value and the seven expression node types the whole engine is built on. Deliberately parallel to CS-02: no dependency between them.
...
- `src/CouponService.Engine/Ast/`
- `tests/CouponService.EngineTests/Ast/`
...
- Add Value and ValueKind exactly as specified in design.md, as a readonly record struct over Number, Text, Bool and List.
- Add the Expr record hierarchy: ConstExpr, FactExpr, LogicalExpr, CompareExpr, MembershipExpr, QuantifierExpr, AggregateExpr, ArithmeticExpr, plus the operator enums and Selector.
- Give Value accessor methods that throw a typed engine exception on a kind mismatch, rather than returning a default. A silent default here would become a wrong price.
- Test kind mismatches, list equality semantics and decimal precision preservation.
...
dotnet test tests/CouponService.EngineTests --filter FullyQualifiedName~Ast
...
```text
Implement ticket CS-03 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-03-value-model-and-expression-ast.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-2.1. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-03-value-model-and-expression-ast from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-03: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-03: Value model and expression AST".
