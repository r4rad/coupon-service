...
Expose the authoritative reserve, confirm and release operations that only the Order API may call.
...
- `tests/CouponService.ApiTests/Reservations/`
...
- Role enforcement, which is CS-18, though the endpoints must be authored ready for it.
...
- POST /v1/reservations returns 201 with an authoritative breakdown, or 409 with reason UsageLimitReached.
- POST /v1/reservations/{orderId}/confirm and /release, both idempotent, 404 for an unknown order.
- These routes must be registered so they can be bound to internal ingress only at deploy time. Do not expose them on the same route prefix as customer endpoints.
- Test the 409 path, both idempotent repeats, and that a reserve response total is independent of anything the caller supplied.
...
dotnet test tests/CouponService.ApiTests --filter FullyQualifiedName~Reservations
...
```text
Implement ticket CS-16 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-16-reservation-endpoints.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-4.1, AC-4.2, AC-4.3, AC-4.5. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-16-reservation-endpoints from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-16: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-16: Reservation endpoints".
