...
| **Wave** | 0 — Foundations |
| **Size** | M |
| **Labels** | `wave-0, area:domain, size:M` |
| **Blocked by** | [CS-01](CS-01-solution-wiring-central-packages-and-build-gates.md) |
| **Blocks** | [CS-09](CS-09-core-effect-handlers-and-the-price-calculator.md) |
...
Establish the money and basket types every later ticket depends on, with rounding behaviour pinned by tests.
...
- `src/CouponService.Domain/`
- `tests/CouponService.UnitTests/Domain/`
...
- Policy AST types, which belong to CS-03.
- Any discount calculation, which belongs to CS-09.
...
- Add Cart, CartLine (line id, pizza id, category, unit price, quantity) and a Money helper exposing the rounding rule in exactly one place.
- Add PriceBreakdown (currency, line totals, subtotal, discount, total), DiscountPlan and LineAllocation as described in design.md.
- Line total is unit price times quantity rounded to 2dp; subtotal is the sum of already-rounded line totals. Encode that ordering, it is observable.
- Add an IClock abstraction plus a SystemClock and a FixedClock for tests.
- Test the worked example from the architecture document: 2 at 9.50 plus 1 at 12.00 gives subtotal 31.00, and 10 percent gives discount 3.10 and total 27.90. Also test that a discount cannot drive a total below zero, and that a midpoint value rounds away from zero.
...
dotnet test tests/CouponService.UnitTests --filter FullyQualifiedName~Domain
...
```text
Implement ticket CS-02 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-02-domain-primitives-and-the-money-contract.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-1.5, AC-1.4, NFR-3. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-02-domain-primitives-and-the-money-contract from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-02: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-02: Domain primitives and the money contract".
