...
| **Size** | L |
| **Labels** | `wave-1, area:engine, area:security, size:L` |
...
Parse a policy condition from JSON into the AST, using the single-key operator dispatch, with budgets enforced as a security control.
...
- `src/CouponService.Engine/Parsing/`
- `tests/CouponService.EngineTests/Parsing/`
...
- Fact existence and type checking, which is CS-06.
- Effect parsing, which is CS-09.
...
- Implement the parser over System.Text.Json.JsonElement following the switch in section 5.6 of docs/solution-architecture.md. An object must have exactly one property; the property name is the operator.
- Support every condition form in the section 5.4 grammar table: constant, fact, all, any, not, the six comparisons, in, between, every, some, sum, count, min, max, and the arithmetic operators.
- Implement ParseBudget with configurable maximum node count and nesting depth, spent on every node, throwing PolicyBudgetException when exceeded.
- Throw PolicySyntaxException carrying a JSON path for the offending node, never a bare message.
- Test each operator round-trips; an object with two keys is rejected; an unknown operator is rejected; a deeply nested and a very wide document are both rejected by budget, before any evaluation happens.
...
dotnet test tests/CouponService.EngineTests --filter FullyQualifiedName~Parsing
...
```text
Implement ticket CS-04 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-04-policy-parser-with-depth-and-node-budgets.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-2.1, AC-2.5. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-04-policy-parser-with-depth-and-node from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-04: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-04: Policy parser with depth and node budgets".
