...
| **Size** | M |
| **Labels** | `wave-1, area:engine, size:M` |
| **Blocked by** | [CS-03](CS-03-value-model-and-expression-ast.md) |
| **Blocks** | [CS-06](CS-06-policy-validator-with-full-error-reporting.md), [CS-07](CS-07-compiler-with-cost-ordering-and-the-evaluation.md) |
...
Provide the typed, cost-classed fact vocabulary that is the engine's only contact with the outside world, and the self-describing manifest generated from it.

## Blocked by

- CS-03 — Value model and expression AST

## Scope — touch only these paths

- `src/CouponService.Engine/Facts/`
- `src/CouponService.Engine/Manifest/`
- `tests/CouponService.EngineTests/Facts/`
...
- Wiring real Cosmos-backed fact resolvers, which happens in CS-12 and CS-23.
...
- Add FactCost, FactDescriptor and IFactRegistry per design.md, with async ValueTask resolvers per decision P-5.
- Register the documented vocabulary: cart.subtotal, cart.lineCount, cart.totalQuantity as Pure; line.category, line.unitPrice, line.quantity as Pure within quantifier scope; customer.confirmedOrderCount and customer.isFirstOrder as Cached; coupon.uses.total and coupon.uses.byCustomer as RemoteRead; time.utcNow, time.localDayOfWeek and time.localHour as Pure from the injected clock.
- Generate an EngineManifest from the registry listing every fact with type and cost, every operator, every effect and the configured budgets. It must be generated, never hand-maintained, so it cannot drift.
- Test that every registered fact appears in the manifest, that a duplicate registration is rejected, and that resolving an unregistered path fails with a typed exception.
...
dotnet test tests/CouponService.EngineTests --filter FullyQualifiedName~Facts
dotnet test CouponService.slnx
```

## Prompt

Confirm CS-03 is merged, then paste this into a fresh Cursor
chat in this repository. Nothing else is needed: everything is either in the prompt or in a
file the prompt names.

```text
Implement ticket CS-05 in this repository, end to end.

Read these first, in order. They are the contract and they override anything you assume:
  1. .github/tickets/CS-05-fact-registry-fact-vocabulary-and-engine-manifest.md
  ...
  3. .kiro/specs/coupon-service/requirements.md
     The full text of AC-6.2. Look each one up and read it. Do not
...
Then:
  1. Create branch ticket/CS-05-fact-registry-fact-vocabulary from the latest main.
  ...
  6. Commit in atomic, granular steps as you go - one logical change each, every commit
     building - with subjects of the form "CS-05: <imperative summary>". Do not squash the
  ...
  7. Push the branch and open a pull request against main titled "CS-05: Fact registry, fact vocabulary and engine manifest".
